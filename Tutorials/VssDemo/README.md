...
GitHub](https://github.com/COVESA/vehicle_signal_specification) is provided as JSON file and part of the container. It is located in: `/opt/vaf/Demo/VssDemo/model/vss/vss.json`.
``` bash
...
Two new files are added to the project folder by this command. `VssInterfaces.json` is the VAF model
file in JSON format. It contains all relevant information as imported from the VSS catalogue.
`vss_interfaces.py` is the Configuration as Code (CaC) support, which is needed to access the model
